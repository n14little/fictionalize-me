...
describe('Lexorank Helper Functions - Integration Tests', () => {
  ...
  describe('lexorank_between function', () => {
    it('should generate a rank between two given ranks', async () => {
      const result = await query('SELECT lexorank_between($1, $2) as rank', [
        '10',
        '20',
      ]);
    ...
    it('should handle empty strings like null values', async () => {
      const result1 = await query('SELECT lexorank_between($1, $2) as rank', [
        '',
        '20',
      ]);

      const result2 = await query('SELECT lexorank_between($1, $2) as rank', [
        '10',
        '',
      ]);

      const result3 = await query('SELECT lexorank_between($1, $2) as rank', [
        '',
        '',
      ]);
  ...
  describe('lexorank_next function', () => {
    it('should generate the next rank after a given rank', async () => {
      const result = await query('SELECT lexorank_next($1) as rank', ['10']);
    ...
    it('should generate a default rank when input is null', async () => {
      const result = await query('SELECT lexorank_next($1) as rank', [null]);
    ...
    it('should generate a default rank when input is empty string', async () => {
      const result = await query('SELECT lexorank_next($1) as rank', ['']);
    ...
    it('should generate increasing ranks', async () => {
      ...
      const result2 = await query('SELECT lexorank_next($1) as rank', [
        row1.rank,
      ]);
      const row2 = result2.rows[0] as LexorankResult;

      const result3 = await query('SELECT lexorank_next($1) as rank', [
        row2.rank,
      ]);
      const row3 = result3.rows[0] as LexorankResult;
  ...
  describe('lexorank_before function', () => {
    it('should generate a rank before a given rank', async () => {
      const result = await query('SELECT lexorank_before($1) as rank', ['10']);
    ...
    it('should generate a default rank when input is null', async () => {
      const result = await query('SELECT lexorank_before($1) as rank', [null]);
    ...
    it('should generate a default rank when input is empty string', async () => {
      const result = await query('SELECT lexorank_before($1) as rank', ['']);
    ...
    it('should generate decreasing ranks', async () => {
      ...
      const result2 = await query('SELECT lexorank_before($1) as rank', [
        row1.rank,
      ]);
      const row2 = result2.rows[0] as LexorankResult;

      const result3 = await query('SELECT lexorank_before($1) as rank', [
        row2.rank,
      ]);
      const row3 = result3.rows[0] as LexorankResult;
  ...
  describe('Integration with existing lexo_priority column', () => {
    it('should generate valid lexorank values for database ordering', async () => {
      ...
      let currentRank = await query('SELECT lexorank_between($1, $2) as rank', [
        null,
        null,
      ]);
      ...
      for (let i = 0; i < 5; i++) {
        const nextRank = await query('SELECT lexorank_next($1) as rank', [
