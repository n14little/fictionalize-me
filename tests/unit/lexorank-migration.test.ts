...
describe('Lexorank Migration', () => {
  it('should verify migration file exists and has correct content', () => {
    ...
    const migrationPath = path.join(__dirname, '../../db/migrations/V19__add_lexorank_column.sql');
    ...
    expect(migrationContent).toContain('ALTER TABLE tasks ADD COLUMN lexo_priority TEXT');
    
    // Verify the migration creates the index
    expect(migrationContent).toContain('CREATE INDEX idx_tasks_lexo_priority ON tasks (lexo_priority)');
  });
  ...
  it('should verify schema.sql includes lexo_priority column', () => {
    ...
    expect(schemaContent).toContain('CREATE INDEX idx_tasks_lexo_priority ON public.tasks USING btree (lexo_priority)');
    
    // Verify the schema includes the drop statement for the index
    expect(schemaContent).toContain('DROP INDEX IF EXISTS public.idx_tasks_lexo_priority');
  });
});