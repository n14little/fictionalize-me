...
test.describe('Journal functionality', () => {
  ...
  test.skip('should be able to create and navigate to journal', async ({
    page,
  }) => {
    await page.goto('/journals/new');

    await expect(
      page.getByRole('heading', { name: /create.*journal/i })
    ).toBeVisible();
    await expect(page.getByLabel(/title/i)).toBeVisible();
    await expect(
      page.getByRole('button', { name: /create|submit|save/i })
    ).toBeVisible();
    ...
    const journalCard = page.getByRole('link', {
      name: new RegExp(journalTitle, 'i'),
    });
    ...
    await expect(
      page.getByRole('heading', { name: new RegExp(journalTitle, 'i') })
    ).toBeVisible();
    await expect(
