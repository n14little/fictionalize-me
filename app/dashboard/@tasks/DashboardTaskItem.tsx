      ...
      <div className="flex items-start gap-2">
        <form action={handleToggleCompletion}>
          ...
          <button
            type="submit"
            disabled={isToggling}
            className={`w-4 h-4 rounded border mt-0.5 ${task.completed 
        ...
        <div className="flex-1 min-w-0">
          <h3 className={`text-sm font-medium ${task.completed ? 'line-through text-gray-400' : 'text-gray-700'}`}>
            {task.title}
          </h3>
          {task.description && (
            <p className={`mt-1 text-xs ${task.completed ? 'text-gray-400' : 'text-gray-500'} line-clamp-2`}>
              {task.description}
            </p>
          )}
